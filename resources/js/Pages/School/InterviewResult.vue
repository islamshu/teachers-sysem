    ...
    <div>
      <div class="flex items-center justify-between mb-8 animate-fade-in-up">
        <div>
          ...
          <p class="text-slate-500 mt-1">تقييم المدرس {{ interview.employment?.teacher?.user?.name }}</p>
        </div>
      ...
      <div class="card p-8 mb-6 animate-fade-in-up">
        <div class="text-center">
          ...
          <div class="mt-4 flex gap-3 justify-center">
            <span v-if="interview.employment?.status === 'hired'" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-emerald-100 text-emerald-700 font-bold text-sm">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
              </svg>
              تم التوظيف
            </span>
            <span v-else-if="interview.employment?.status === 'rejected'" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-red-100 text-red-700 font-bold text-sm">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="card p-6 mb-6 animate-fade-in-up">
        ...
        <div class="flex items-center gap-4">
          <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-100 to-surface-100 overflow-hidden flex-shrink-0">
            <img v-if="interview.employment?.teacher?.photo" :src="`/storage/${interview.employment.teacher.photo}`" :alt="interview.employment?.teacher?.user?.name" class="w-full h-full object-cover" />
            <div v-else class="w-full h-full flex items-center justify-center text-xl font-bold text-primary-600">
              {{ interview.employment?.teacher?.user?.name?.charAt(0) }}
            </div>
          </div>
          <div>
            <h3 class="text-lg font-bold text-slate-900">{{ interview.employment?.teacher?.user?.name }}</h3>
            <p class="text-sm text-slate-500">{{ interview.employment?.subject?.name }}</p>
          </div>
        </div>
      </div>
      ...
      <!-- Actions -->
      <div v-if="interview.employment?.status === 'interviewed'" class="card p-6 animate-fade-in-up flex gap-3">
        <button @click="hireTeacher" class="btn-primary flex-1 py-3 text-base" :disabled="hiringForm.processing">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
          </svg>
          {{ hiringForm.processing ? 'جاري...' : 'توظيف' }}
        </button>
        <button @click="rejectTeacher" class="btn-ghost flex-1 py-3 text-base text-red-600 hover:bg-red-50">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
          </svg>
          رفض
...
const hireTeacher = () => {
  hiringForm.patch(`/school/interviews/${props.interview.id}/hire`, {
    preserveScroll: true,
  })
}
...
const rejectTeacher = () => {
  if (confirm('هل أنت متأكد من رفض هذا المدرس؟')) {
    hiringForm.patch(`/school/interviews/${props.interview.id}/reject`, {
      preserveScroll: true,
    })
  }
}
