    ...
    <div>
      <div class="flex items-center justify-between mb-8 animate-fade-in-up">
        <div>
          ...
          <p class="text-slate-500 mt-1">تقييم الموظف {{ invitation.employee?.name }}</p>
        </div>
          ...
          <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-100 to-surface-100 overflow-hidden flex-shrink-0">
              <div class="w-full h-full flex items-center justify-center text-xl font-bold text-primary-600">
                {{ invitation.employee?.name?.charAt(0) }}
              </div>
            </div>
            <div>
              <h3 class="text-lg font-bold text-slate-900">{{ invitation.employee?.name }}</h3>
              <div class="flex flex-wrap gap-1.5 mt-1">
                <span v-for="role in invitation.employee?.roles" :key="role.id" class="text-xs px-2 py-0.5 rounded-full bg-primary-100 text-primary-700">{{ role.name }}</span>
              </div>
            </div>
          </div>
        </div>
...
const submitInterview = () => {
  form
    ...
    .post(`/school/employee-invitations/${props.invitation.id}/interview`, {
      preserveScroll: true,
    })
}
</script>
