  ...
  <DashboardLayout>
    <div>
      ...
      <div class="flex items-center justify-between mb-8 animate-fade-in-up">
        <div>
          <h1 class="text-3xl font-extrabold text-slate-900">إدارة المواد</h1>
          <p class="text-slate-500 mt-1">إضافة وتعديل وحذف المواد الدراسية</p>
        </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-surface-50 border-b border-surface-200">
                ...
                <th class="px-6 py-4 text-right text-sm font-bold text-slate-700">اسم المادة</th>
                <th class="px-6 py-4 text-center text-sm font-bold text-slate-700">الإجراءات</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-surface-100">
              <tr v-for="(subject, index) in filteredList" :key="subject.id" class="hover:bg-primary-50/50 transition-colors">
                ...
                <td class="px-6 py-4">
                  <div class="flex gap-2 justify-center">
                    <button @click="openEditModal(subject)" class="p-2 rounded-xl bg-primary-100 text-primary-700 hover:bg-primary-200 transition-all duration-200" title="تعديل">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      </svg>
                    </button>
                    <button @click="confirmDelete(subject)" class="p-2 rounded-xl bg-red-100 text-red-700 hover:bg-red-200 transition-all duration-200" title="حذف">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <tr v-if="filteredList.length === 0">
                <td colspan="3" class="px-6 py-12 text-center text-slate-500">لا توجد مواد مضافة بعد</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="closeModal">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 animate-scale-in">
            <div class="flex items-center gap-3 mb-6">
              ...
              <h3 class="text-xl font-bold text-slate-900">{{ editingSubject ? 'تعديل المادة' : 'إضافة مادة جديدة' }}</h3>
            </div>

            <form @submit.prevent="submitForm">
              <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1.5">اسم المادة <span class="text-red-500">*</span></label>
                <input
                  v-model="form.name"
                  type="text"
                  class="input-base"
              ...
              <div class="flex gap-3 mt-8">
                <button type="submit" class="btn-primary flex-1" :disabled="form.processing">
                  {{ form.processing ? 'جاري الحفظ...' : (editingSubject ? 'تحديث' : 'إضافة') }}
                </button>
...
const filteredList = computed(() => props.subjects.filter(item => matchesSearch(item, search.value)))
