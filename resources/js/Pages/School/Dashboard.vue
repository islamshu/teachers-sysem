        ...
        <div class="relative px-4 md:px-8 py-6 md:py-10">
          <div class="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 mb-3">
            ...
            <div class="text-center sm:text-right">
              <h1 class="text-xl sm:text-2xl md:text-3xl font-extrabold text-white">مرحباً، {{ $page.props.auth.user?.name }}</h1>
              <p class="text-primary-200 text-sm sm:text-base">لوحة تحكم المدرسة</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="card animate-fade-in-up animate-delay-100">
            <template v-if="!profile">
              <div class="p-10 text-center">
                ...
                <h3 class="text-2xl font-bold text-slate-900 mb-2">لم تقم بإضافة بيانات المدرسة بعد</h3>
                <p class="text-slate-500 mb-8 max-w-md mx-auto">قم بإضافة بيانات المدرسة كاملة ليتم عرضها ويمكن للإدارة الموافقة عليها</p>
                <Link
                ...
                <div class="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6">
                  <div class="relative flex-shrink-0">
                    <div class="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-gradient-to-br from-primary-100 to-surface-100 overflow-hidden shadow-lg shadow-primary-500/10">
                      <img v-if="profile.logo" :src="`/storage/${profile.logo}`" :alt="profile.school_name" class="w-full h-full object-cover" />
                  ...
                  <div class="flex-1 min-w-0 w-full">
                    <div class="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-3 mb-4">
                      <div class="text-center sm:text-right w-full">
                        <h3 class="text-xl md:text-2xl font-bold text-slate-900">{{ profile.school_name }}</h3>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                      <div class="p-3 rounded-xl bg-primary-50">
                        <p class="text-xs text-primary-600 font-semibold mb-0.5">السجل التجاري</p>
                        <p class="text-sm font-bold text-slate-900 dir-ltr">{{ profile.commercial_register || '-' }}</p>
                      </div>
                      <div class="p-3 rounded-xl bg-warm-50">
                        <p class="text-xs text-warm-600 font-semibold mb-0.5">الهاتف</p>
                        <p class="text-sm font-bold text-slate-900">{{ profile.phone || '-' }}</p>
                      </div>
                      <div class="p-3 rounded-xl bg-emerald-50">
                        <p class="text-xs text-emerald-600 font-semibold mb-0.5">مدير المدرسة</p>
                        <p class="text-sm font-bold text-slate-900 truncate">{{ profile.principal_name || '-' }}</p>
                      </div>
                      <div class="p-3 rounded-xl bg-rose-50">
                        <p class="text-xs text-rose-600 font-semibold mb-0.5">العنوان</p>
                        <p class="text-sm font-bold text-slate-900 truncate">{{ profile.address || '-' }}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="card p-6 animate-fade-in-up animate-delay-200">
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <span class="font-medium">إضافة بيانات المدرسة</span>
              </Link>
              ...
              >
                ...
                <span class="font-medium">تعديل بيانات المدرسة</span>
              </Link>
            </div>
          </div>
        </div>
...
const page = usePage()
const profile = computed(() => page.props.auth.user?.school_profile)
...
const statusDescription = computed(() => {
  const map = {
    pending: 'بيانات المدرسة قيد المراجعة من قبل الإدارة، سيتم إخطارك عند القبول',
    approved: 'تم قبول بيانات المدرسة وستظهر في المنصة',
    rejected: 'لم يتم قبول بيانات المدرسة، يمكنك تعديلها وإعادة التقديم',
  }
  return map[profile.value?.status] || ''
})
</script>
